...
inline tcalkey& next( tcalkey& s )
{
  ...
  else if(T_DENITR == s) { return s = T_CMAX; }
...
inline tcalkey& prev( tcalkey& s )
{
  ...
  else if(T_CMAX == s) { return s = T_DENITR; }
...
inline tcalkey& jump(tcalkey& s)
{
  ...
  if((T_CMAX <= s) && (T_DENITR >= s)) { return s = T_SLA; }
...
inline tcalkey& back(tcalkey& s)
{
  ...
  if((T_CMAX <= s) && (T_DENITR >= s)) { return s = T_GDDMIN; }
...
//    if( spinflag == 2 ) { cdyear = (de_startyear + 1 - clm.startyr) + (xdyr%de_nyears); }
    if( spinflag == 2 ) { cdyear = (de_startyear - clm.startyr) + (xdyr%de_nyears); }
    else { cdyear = 0; }
...
//// End of 9 agfile variables    
    ...
    case T_DENITR :  move( VPARAMS_ROW+2, 20 );
...
//// End of 9 agfile variables
    ...
    case T_DENITR :  tem.soil.setDENITR( (tcalvar[T_DENITR] * factor),
                                       tem.veg.cmnt );
                    break;
//// End of 19 datfile variables
...
//  
  ...
  for( dm = 0; dm < CYCLE; ++dm )
  {
    ...
    climate[clm.I_TAIRD][dm][pdyr] = climate[clm.I_TAIR][dm][pdyr]
                            + 12.0*climate[clm.I_TRANGE][dm][pdyr]/climate[clm.I_DAYL][dm][pdyr]
                            * 1.732 / (2.0*3.14159) * sin( 3.14159 * climate[clm.I_DAYL][dm][pdyr] / 24.0 );

    // Nighttime air temperature

    climate[clm.I_TAIRN][dm][pdyr] = climate[clm.I_TAIR][dm][pdyr]
                            - 12.0*climate[clm.I_TRANGE][dm][pdyr]/(24.0 - climate[clm.I_DAYL][dm][pdyr])
                            * 1.732 / (2.0*3.14159) * sin( 3.14159 * climate[clm.I_DAYL][dm][pdyr] / 24.0 );
...
//
    ...
    climate[clm.I_VPDD][dm][pdyr] = (10.*0.61078*exp((17.27*climate[clm.I_TAIRD][dm][pdyr])
                                                  /(climate[clm.I_TAIRD][dm][pdyr]+237.3)))-vprdat[dyr].mon[dm];
    if(climate[clm.I_VPDD][dm][pdyr] < ZERO) { climate[clm.I_VPDD][dm][pdyr] = ZERO; }


    climate[clm.I_VPDN][dm][pdyr] = (10.*0.61078*exp((17.27*climate[clm.I_TAIRN][dm][pdyr])
                                                  /(climate[clm.I_TAIRN][dm][pdyr]+237.3)))-vprdat[dyr].mon[dm];
    if(climate[clm.I_VPDN][dm][pdyr] < ZERO) { climate[clm.I_VPDN][dm][pdyr] = ZERO; }
