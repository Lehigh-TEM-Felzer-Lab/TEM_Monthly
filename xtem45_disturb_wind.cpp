...
void initRun(void)
{
  ...
  if (telmnt[0].clm.tws10flag == 1)
  {
    flog1 << " transient wind dataset: " << telmnt[0].clm.iws10fname + telmnt[0].clm.iws10end << endl;
  }
  else
  {
    flog1 << " long-term average wind dataset: " << telmnt[0].clm.iws10fname << endl;
  }
...
void updateTCLMGridCell(const int &pdyr)
{
  ...
  telmnt[0].year = telmnt[0].clm.startyr - totsptime
                   //                   - 1
                   + pdyr;
...
//  Pass disturbance information to telmnt[0]
  ...
  for (ichrt = 0; ichrt < telmnt[0].maxcohorts; ++ichrt)
  {
    ...
    cout << "lulc type = " << ichrt << " "
         << " " << telmnt[0].cohort[ichrt].cmnt << " " << telmnt[0].cohort[ichrt].agcmnt << " " << lulcdat[tstyr][ichrt].agstate << endl;
  }
};
