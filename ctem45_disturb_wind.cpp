...
enum tcalkey
{
  ...
  T_LSCMIN,
  T_RHOSTEM,
  // 10 "conductance-related" variables
  T_OPTCN0,
  T_OPTCN1,
  T_OPTCN2,
  T_OPTCN3,
...
{
  if ((T_TOL <= s) && (T_MANFLAG >= s))
  {
    return s = T_GDDMIN;
  }
  if ((T_GDDMIN <= s) && (T_IRRGFLAG >= s))
  {
    return s = T_CMAX;
  }
  if ((T_CMAX <= s) && (T_DENITR >= s))
  {
    return s = T_SLA;
  }
  if ((T_SLA <= s) && (T_MOPT >= s))
  ...
  if ((T_GROOT <= s) && (T_RHOSTEM >= s))
  {
    return s = T_OPTCN0;
  }
  if ((T_OPTCN0 <= s) && (T_OPTCN9 >= s))
  {
    return s = T_OPTH2O0;
  }
  if ((T_OPTH2O0 <= s) && (T_OPTH2O7 >= s))
  {
    return s = T_TOL;
  }
  else
...
int main()
{
  ...
  cout << "finished initRun" << endl;

  if (spinflag == 2)
  {
    firstyear = (de_startyear + 1 - clm.startyr);
  }
  else
  {
    firstyear = 0;
  ...
  while (xdyr > -10)
  {
    ...
    if (spinflag == 2)
    {
      cdyear = (de_startyear + 1 - clm.startyr) + (xdyr % de_nyears);
    }
    else
    {
      cdyear = 0;
...
void autocalibrate(void)
{
  ...
  if (xdyr % ((int)(5 * tem.tauavg)) == 0)
  {
    ...
    case 9: // check that npp ~ npp_t and gpp ~gpp_t
      if ((fabs(tem.veg.yrgpp - gpp_t) / gpp_t < autocal_tol) && (fabs(tem.veg.yrnpp - npp_t) / npp_t < autocal_tol))
      {
        calstep++;
        adj_fac = init_adj_fac;
        target_error[10] = 0;
...
void displayCalibPar(tcalkey &tcal)
{
  ...
  case T_DENITR:
    ...
    printw("DENITR = %8.2lf                  ",
           tem.soil.getDENITR(tem.veg.cmnt));
    break;
    //// End of 19 datfile variables

  case T_SLA:
    move(VPARAMS_ROW + 3, 20);
...
void initRun(void)
{
  ...
  if (2 == spinflag)
  {
    flog1 << " startyear for dynamic equilibration: " << de_startyear << endl;
  }

  de_nyears = tem.goxml.getXMLint(tem.gofile, "gofile", "de_nyears");
  if (2 == spinflag)
  {
    flog1 << " number of years for dynamic equilibration loop: " << de_nyears << endl;
  }
  ...
  if (clm.sradflag == 0)
  {
    flog1 << " spatially explicit PAR data " << ((clm.parflag == 1) ? "will" : "will not") << " be used " << endl;
  }
  ...
  if ((clm.cldflag == 1) && (clm.sradflag == 1))
  {
    if (clm.tcldsflag == 1)
    {
      flog1 << " filename for cloudiness data input: " << clm.icldsfname + clm.icldsend << endl;
    }
    else
    {
      flog1 << " filename for cloudiness data input: " << clm.icldsfname << endl;
    }
  }
  ...
  if ((clm.cldflag == 0) && (clm.sradflag == 1))
  {
    if (clm.tcldsflag == 1)
    {
      flog1 << " filename for solar radiation data input: " << clm.inirrfname + clm.inirrend << endl;
    }
    else
    {
      flog1 << " filename for solar radiation data input: " << clm.inirrfname << endl;
    }
  }
  ...
  if (clm.ttairflag == 1)
  {
    flog1 << " transient tair dataset: " << clm.itairfname + clm.itairend << endl;
  }
  else
  {
    flog1 << " long-term average tair dataset: " << clm.itairfname << endl;
  }
  ...
  if (clm.tprecflag == 1)
  {
    flog1 << " transient prec dataset: " << clm.iprecfname + clm.iprecend << endl;
  }
  else
  {
    flog1 << " long-term average prec dataset: " << clm.iprecfname << endl;
  }
  ...
  if (clm.ttrangeflag == 1)
  {
    flog1 << " transient trange dataset: " << clm.itrangefname + clm.itrangeend << endl;
  }
  else
  {
    flog1 << " long-term average trange dataset: " << clm.itrangefname << endl;
  }
  ...
  if (clm.tvprflag == 1)
  {
    flog1 << " transient vpr dataset: " << clm.ivprfname + clm.ivprend << endl;
  }
  else
  {
    flog1 << " long-term average vpr dataset: " << clm.ivprfname << endl;
  }
  ...
  if (clm.to3flag == 1)
  {
    flog1 << " transient o3 dataset: " << clm.io3fname + clm.io3end << endl;
  }
  else
  {
    flog1 << " long-term average o3 dataset: " << clm.io3fname << endl;
  }
  ...
  if (clm.tndepflag == 1)
  {
    flog1 << " transient ndep dataset: " << atmdep.ndepfname + atmdep.ndepend << endl;
  }
  else
  {
    flog1 << " long-term average ndep dataset: " << atmdep.ndepfname << endl;
  }
  ...
  if (clm.tws10flag == 1)
  {
    flog1 << " transient ws10 dataset: " << clm.iws10fname + clm.iws10end << endl;
  }
  else
  {
    flog1 << " long-term average ws10 dataset: " << clm.iws10fname << endl;
  }
  ...
  if (tem.equil == 1)
  {
    flog1 << " model will check for equilibrium after running for " << tem.strteq << " years " << endl;
  }

  tem.rheqflag = tem.goxml.getXMLint(tem.gofile, "gofile", "rheqflag");
  if (tem.equil == 1)
  {
    flog1 << " decomposition " << ((tem.rheqflag == 1) ? "must" : "is not required to") << " equilibrate " << endl;
  }

  tem.wtol = tem.goxml.getXMLdouble(tem.gofile, "gofile", "wtol");
  if (tem.equil == 1)
  {
    flog1 << " absolute tolerance for the water cycle: " << tem.wtol << endl;
  }

  tem.ctol = tem.goxml.getXMLdouble(tem.gofile, "gofile", "ctol");
  if (tem.equil == 1)
  {
    flog1 << " absolute tolerance for the carbon cycle: " << tem.ctol << endl;
  }

  tem.ntol = tem.goxml.getXMLdouble(tem.gofile, "gofile", "ntol");
  if (tem.equil == 1)
  {
    flog1 << " absolute tolerance for the nitrogen cycle: " << tem.ntol << endl;
  }

  tem.maxyears = tem.goxml.getXMLint(tem.gofile, "gofile", "maxyears");
  if (tem.equil == 1)
  {
    flog1 << " maximum number of years for the model to run before initialization is complete: " << tem.maxyears << " years " << endl;
  }
  ...
  if (tem.equil == 1)
  {
    flog1 << " maximum number of attempts to reach a solution: " << tem.maxnrun << endl;
  }
...
void resetInitState(void)
{
  ...
  for (dm = 0; dm < CYCLE; ++dm)
  {
    ...
    avgtair += climate[clm.I_TAIRD][dm][firstyear] / 12.0;

    if (dm == 0)
    {
      tem.veg.setTOPT(tem.veg.getTOPTMIN(tem.veg.cmnt));
    }
    ...
    if (dm == 0)
    {
      tem.veg.setTOPTMIC(tem.veg.getTOPTMIN(tem.veg.cmnt));
    }
    if (climate[clm.I_TAIRD][dm][0] > tem.veg.getTOPTMIC())
    {
      tem.veg.setTOPTMIC(climate[clm.I_TAIRD][dm][firstyear]);
    }
  }
...
void setCalibVar(void)
{
  ...
  tcalvar[T_DENITR] = tem.soil.getDENITR(tem.veg.cmnt);
                     ...
                     FILE *felev)
{
  ...
  if (-1 == gisend)
  {
    rflog1 << "Ran out of Soil texture data" << endl
           << endl;
  ...
  if (gisend == -1)
  {
    rflog1 << "Ran out of Elevation data" << endl
           << endl;
                    ...
                    int &t)
{
  ...
  {
  case T_TOL:
    tem.tol = tcalvar[T_TOL] * factor;
    break;

  case T_TAVG:
    tem.tauavg = tcalvar[T_TAVG] * factor;
    break;

  case T_CO2:
    tem.atms.setCO2LEVEL((tcalvar[T_CO2] * factor));
    for (dm = 0; dm < CYCLE; ++dm)
    {
      climate[clm.I_CO2][dm][0] = tem.atms.getCO2LEVEL();
    }
  ...
  case T_GDDMIN:
    tem.ag.setGDDMIN((tcalvar[T_GDDMIN] * factor),
                     tem.ag.cmnt);
    break;

  case T_GDDSEED:
    tem.ag.setGDDSEED((tcalvar[T_GDDSEED] * factor),
                      tem.ag.cmnt);
    break;

  case T_GDDHARVST:
    tem.ag.setGDDHARVST((tcalvar[T_GDDHARVST] * factor),
                        tem.ag.cmnt);
    break;

  case T_TKILL:
    tem.ag.setTKILL((tcalvar[T_TKILL] * factor),
                    tem.ag.cmnt);
    break;
  ...
  case T_CMAX:
    tem.veg.setCMAX((tcalvar[T_CMAX] * factor));
    break;

  case T_KRA:
    tem.veg.setKRA((tcalvar[T_KRA] * factor),
                   tem.veg.cmnt);
    break;

  case T_KDC:
    tem.microbe.setKDC((tcalvar[T_KDC] * factor));
    break;

  case T_TAULEAF:
    tem.veg.setTAULEAF((tcalvar[T_TAULEAF] * factor),
                       tem.veg.cmnt);
    break;

  case T_TAUSTEMA:
    tem.veg.setTAUSAPWOOD((tcalvar[T_TAUSTEMA] * factor),
                          tem.veg.cmnt);
    break;

  case T_TAUSTEMI:
    tem.veg.setTAUHEARTWOOD((tcalvar[T_TAUSTEMI] * factor),
                            tem.veg.cmnt);
    break;

  case T_TAUROOT:
    tem.veg.setTAUROOT((tcalvar[T_TAUROOT] * factor),
                       tem.veg.cmnt);
    break;

  case T_TAUSEED:
    tem.veg.setTAUSEED((tcalvar[T_TAUSEED] * factor),
                       tem.veg.cmnt);
    break;

  case T_NMAX:
    tem.veg.setNMAX((tcalvar[T_NMAX] * factor));
    break;

  case T_MNUP:
    tem.microbe.setNUP((tcalvar[T_MNUP] * factor));
    break;

  case T_CNSOL:
    tem.microbe.setCNSOIL((tcalvar[T_CNSOL] * factor),
                          tem.veg.cmnt);
    break;

  case T_CNLTR:
    tem.veg.setCNLTR((tcalvar[T_CNLTR] * factor),
                     tem.veg.cmnt);
    break;

  case T_NLOSS:
    tem.soil.setNLOSS((tcalvar[T_NLOSS] * factor),
                      tem.veg.cmnt);
    break;

  case T_DENITR:
    tem.soil.setDENITR((tcalvar[T_DENITR] * factor),
                       tem.veg.cmnt);
    break;
    //// End of 19 datfile variables

  case T_SLA:
    tem.veg.setSLA((tcalvar[T_SLA] * factor),
                   tem.veg.cmnt);
    break;

  case T_MOPT:
    tem.microbe.setMOISTOPT((tcalvar[T_MOPT] * factor),
                            tem.veg.cmnt);
    break;
  ...
  case T_GROOT:
    tem.veg.pen.setGROOT((tcalvar[T_GROOT] * factor),
                         tem.veg.cmnt);
    break;

  case T_KSTEM:
    tem.veg.pen.setKSTEM((tcalvar[T_KSTEM] * factor),
                         tem.veg.cmnt);
    break;

  case T_CDLEAF:
    tem.veg.pen.setCDLEAF((tcalvar[T_CDLEAF] * factor),
                          tem.veg.cmnt);
    break;

  case T_WLEAF:
    tem.veg.pen.setWLEAF((tcalvar[T_WLEAF] * factor),
                         tem.veg.cmnt);
    break;

  case T_GSMIN:
    tem.veg.pen.setGSMIN((tcalvar[T_GSMIN] * factor),
                         tem.veg.cmnt);
    break;

  case T_GSA:
    tem.veg.pen.setGSA((tcalvar[T_GSA] * factor),
                       tem.veg.cmnt);
    break;

  case T_KEXT:
    tem.veg.pen.setKEXT((tcalvar[T_KEXT] * factor),
                        tem.veg.cmnt);
    break;

  case T_ALBEDO:
    tem.veg.pen.setALBEDO((tcalvar[T_ALBEDO] * factor),
                          tem.veg.cmnt);
    break;

  case T_LSCMIN:
    tem.veg.pen.setLSCMIN((tcalvar[T_LSCMIN] * factor),
                          tem.veg.cmnt);
    break;

  case T_RHOSTEM:
    tem.veg.pen.setRHOSTEM((tcalvar[T_RHOSTEM] * factor),
                           tem.veg.cmnt);
    break;
...
void updateTCLMGridCell(const int &pdyr)
{
  ...
  if (0 == pdyr)
  {
    ...
    if (1 == clm.tws10flag)
    {
      for (dyr = 0; dyr < (transtime); ++dyr)
      {
        ...
        if (-1 == gisend)
        {
          cout << "Ran out of Wind data" << endl
               << endl;
          flog1 << "Ran out of Wind data" << endl
                << endl;
  ...
  for (dm = 0; dm < CYCLE; ++dm)
  {
    climate[clm.I_CO2][dm][pdyr] = clm.getCO2LEVEL();
  }
  ...
  for (dm = 0; dm < CYCLE; ++dm)
  {
    ...
    climate[clm.I_TAIR][dm][pdyr] = tairdat[dyr].mon[dm];
  }
  ...
  for (dm = 0; dm < CYCLE; ++dm)
  {
    ...
    climate[clm.I_DAYL][dm][pdyr] = tem.atms.daylength(tairdat[pdyr].row, dm);

    // Temperature Range

    climate[clm.I_TRANGE][dm][pdyr] = trangedat[dyr].mon[dm];

    // Daytime air temperature

    climate[clm.I_TAIRD][dm][pdyr] = climate[clm.I_TAIR][dm][pdyr] + 12.0 * climate[clm.I_TRANGE][dm][pdyr] / climate[clm.I_DAYL][dm][pdyr] * 1.732 / (2.0 * 3.14159) * sin(3.14159 * climate[clm.I_DAYL][dm][pdyr] / 24.0);

    // Nighttime air temperature

    climate[clm.I_TAIRN][dm][pdyr] = climate[clm.I_TAIR][dm][pdyr] - 12.0 * climate[clm.I_TRANGE][dm][pdyr] / (24.0 - climate[clm.I_DAYL][dm][pdyr]) * 1.732 / (2.0 * 3.14159) * sin(3.14159 * climate[clm.I_DAYL][dm][pdyr] / 24.0);

    // Precipitation

    climate[clm.I_PREC][dm][pdyr] = precdat[dyr].mon[dm];

    // Rain and Snowfall

    clm.precsplt(climate[clm.I_PREC][dm][pdyr],
                 climate[clm.I_TAIR][dm][pdyr],
                 climate[clm.I_RAIN][dm][pdyr],
                 climate[clm.I_SNWFAL][dm][pdyr]);

    //
    //  Calculate vapor pressure deficit from vapor pressure
    //

    climate[clm.I_VPR][dm][pdyr] = vprdat[dyr].mon[dm];

    climate[clm.I_VPDD][dm][pdyr] = (10. * 0.61078 * exp((17.27 * climate[clm.I_TAIRD][dm][pdyr]) / (climate[clm.I_TAIRD][dm][pdyr] + 237.3))) - vprdat[dyr].mon[dm];
    if (climate[clm.I_VPDD][dm][pdyr] < ZERO)
    {
      climate[clm.I_VPDD][dm][pdyr] = ZERO;
    }

    climate[clm.I_VPDN][dm][pdyr] = (10. * 0.61078 * exp((17.27 * climate[clm.I_TAIRN][dm][pdyr]) / (climate[clm.I_TAIRN][dm][pdyr] + 237.3))) - vprdat[dyr].mon[dm];
    if (climate[clm.I_VPDN][dm][pdyr] < ZERO)
    {
      climate[clm.I_VPDN][dm][pdyr] = ZERO;
    }

    // AOT40 ozone index

    climate[clm.I_AOT40][dm][pdyr] = o3dat[dyr].mon[dm];
  ...
  if (tairdat[pdyr].row >= 0.0)
  {
    tem.veg.hemisphere = 1;
  }
  else
  {
    tem.veg.hemisphere = 0;
  }
  ...
  if (1 == clm.sradflag)
  {
    ...
    for (dm = 0; dm < CYCLE; ++dm)
    {
      climate[clm.I_GIRR][dm][pdyr] = clm.xgirr(lat, dm, clm.yrsumday);
    }
  }
  else
  {
    for (dm = 0; dm < CYCLE; ++dm)
    {
      climate[clm.I_GIRR][dm][pdyr] = girrdat[dyr].mon[dm] * Watts2cal;
    }
  }
  ...
  for (dm = 0; dm < CYCLE; ++dm)
  {
    if (1 == clm.cldflag)
    {
      climate[clm.I_CLDS][dm][pdyr] = cldsdat[dyr].mon[dm];

      climate[clm.I_NIRR][dm][pdyr] = clm.xnirr(climate[clm.I_CLDS][dm][pdyr], climate[clm.I_GIRR][dm][pdyr]);
    }
    else
    {
      climate[clm.I_NIRR][dm][pdyr] = nirrdat[dyr].mon[dm] * Watts2cal;

      climate[clm.I_CLDS][dm][pdyr] = clm.mkclds(climate[clm.I_GIRR][dm][pdyr], climate[clm.I_NIRR][dm][pdyr]);
    }

    if (0 == clm.parflag)
    {

      climate[clm.I_PAR][dm][pdyr] = clm.xpar(climate[clm.I_CLDS][dm][pdyr], climate[clm.I_NIRR][dm][pdyr]);
    }
    else
    {
      climate[clm.I_PAR][dm][pdyr] = pardat[dyr].mon[dm] * Watts2cal;
    }
  }
  ...
  if (pdyr == firstyear)
  {
    for (dm = 0; dm < CYCLE; ++dm)
    {
      avgprec += climate[clm.I_PREC][dm][pdyr] / 12.0;
      avgtair += climate[clm.I_TAIR][dm][pdyr] / 12.0;
      avgtaird += climate[clm.I_TAIRD][dm][pdyr] / 12.0;
    }
