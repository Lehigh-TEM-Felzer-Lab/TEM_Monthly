                          ...
                          const double &windspeed)
{
  ...
  */
  ...
  {
    cabove = 100.0;
  } // correction to avoid fatal errors below -- no vegC greater than 100 kg m^-2 allowed

  if (cabove > 0.00001)
  {
    zh = href[pdcmnt] * pow(cabove, kallom[pdcmnt]);
  }
  else
  {
    zh = href[pdcmnt] * pow(0.00001, kallom[pdcmnt]);
  }
  ...
  if (lai < 0.01)
  {
    xcan = pen.getCDLEAF(pdcmnt) * 0.01;
  }
  ...
  if (ua < 0.0)
  {
    cout << " fatal error! ua < 0 " << ua << " " << za << " " << zh
         << " " << zd << " " << zo << " " << cabove << " " << labile.carbon << endl;
  }

  ustar = k_karman * ua / log((za - zd) / zo);
  //  cout << "diag = " << ustar << " " << ua << " " << za << " " << zd << " " << zo << " " << windspeed << endl;
  pen.setUSTAR(ustar);
  if (ustar < 0.0)
  {
    cout << " fatal error! u* < 0 " << ustar << endl;
  }

  if (zh > 1.0)
  {
    n_eddy = 2.306 + 0.194 * zh;
  }
  if (zh > 10.0)
  {
    n_eddy = 4.25;
  ...
  //  cout << "diag = " << pen.getR_AS() << " " << zh << " " << n_eddy << " " << zd << " " << zo << " " << ustar << endl;
  ...
  */
  ...
  case I_STRESS_DEC:
    ...
    if (phi == 1)
    {
      ...
      mcc_l = 1.25 / (12.0 * rphi) * 12.0 * pndays / 365.0; // BSF COMBO
    }
    // else if( (12.0*rphi > 11.5)||(phicnt > 11.0))
    //{
    //  // let vegetation obey evergreen rules
  ...
  case I_COLD_DEC:
    ...
    // cout << "fleafc = " << fleafc << " " << mb_l << " " << mcm_l << " " << mcc_l << endl;
    // stem net benefit:cost ratio
    if ((mb_s - (mcm_s + mcc_s)) > ZERO && (mcm_s + mcc_s) > ZERO)
    {
      fsapwoodc = ((mb_s - (mcm_s + mcc_s)) / (mcm_s + mcc_s)) * (12.0 * pndays / 365.0);
  ...
  case I_STRESS_DEC:
    ...
    if ((mb_l - (mcm_l + mcc_l)) > ZERO && (mcm_l + mcc_l) > ZERO)
    {
      fleafc = 0.75 * ((mb_l - (mcm_l + mcc_l)) / (mcm_l + mcc_l)) * (12.0 * pndays / 365.0);
      //    fleafc =  ((mb_l-(mcm_l+mcc_l))/(mcm_l+mcc_l))*(12.0*pndays/365.0);
    }
    else
    {
    ...
    if ((mb_s - (mcm_s + mcc_s)) > ZERO && (mcm_s + mcc_s) > ZERO)
    {
      fsapwoodc = 0.75 * ((mb_s - (mcm_s + mcc_s)) / (mcm_s + mcc_s)) * (12.0 * pndays / 365.0);
      //    fsapwoodc = ((mb_s-(mcm_s+mcc_s))/(mcm_s+mcc_s))*(12.0*pndays/365.0);
    }
    else
    {
    ...
    if ((mb_r - (mcm_r + mcc_r)) > ZERO && (mcm_r + mcc_r) > ZERO)
    {
      frootc = 0.75 * ((mb_r - (mcm_r + mcc_r)) / (mcm_r + mcc_r)) * (12.0 * pndays / 365.0);
      //    frootc = ((mb_r-(mcm_r+mcc_r))/(mcm_r+mcc_r))*(12.0*pndays/365.0);
    }
    else
    {
  ...
  if (ftotc > ZERO)
  {
    ...
    // cout << "leaf.allocic = " << leaf.allocic << " " << labile.carbon << " " << fleafc << endl;
    ...
    //    cout << "allocin = " << leaf.allocin << " " << labile.nitrogen << " " << fleafn << endl;
  ...
  if ((leaf.rm + sapwood.rm + root.rm + labile.rm) <= labile.carbon)
  {
    ...
    {
      // Windfall allocation
      extra = labile.carbon - 0.67 * (fsaplive[pdcmnt] * sapwood.carbon + root.carbon + leaf.carbon);
      ...
      {
      case I_EVERGREEN:
        if (fleafc > fsapwoodc && fleafc > frootc)
        {
          pleaflc = 1.0 - pseedc;
        }
        else if (fsapwoodc > fleafc && fsapwoodc > frootc)
        {
          psapwoodlc = 1.0 - pseedc;
        }
        else if (frootc > fleafc && frootc > fsapwoodc)
        {
          prootlc = 1.0 - pseedc;
        }
        else
      ...
      // cout << "windfall = " << leaf.allocc << endl;
  ...
  if (nfeed == 1)
  {
    //   cout << "Nlimitation = " << leaf.allocn << " " << cnleaf[pdcmnt] << " " << leaf.allocc << endl;
    ...
    if (leaf.allocn * cnleaf[pdcmnt] < leaf.allocc) // n-limited
    {
      ...
      //      cout << "nlimited = " << leaf.allocc << endl;
    }
    else // c-limited
    {
  ...
  if (lclimflag == 0)
  {
    ...
    root.allocc += root.rm;
    //    cout << "not carbo-limited = " << leaf.allocc << " " << leaf.rm << endl;
  }
  else
  {
  ...
  cnprod = leaf.allocc + sapwood.allocc + root.allocc + seed.allocc + leaf.rg + sapwood.rg + root.rg + seed.rg + labile.rm;
  ...
  // cout << "leaf.allocc = " << leaf.allocc << endl;
  ...
  // cout << "costs = " << rmt << " " << mcm_l << " " << pndays << " " << mcc_l << endl;

  // cout << "diag = " << mb_s <<  " " << fsapwoodc << " " << cnsapwood[pdcmnt] << " " << tausapwood[pdcmnt] << " " << labile.carbon << " " << extra << " " << pndays << " " << sapwood.rm << " " << sapwood.allocc << endl;
  // cout << "diag = " << fsaplive[pdcmnt] << " " << sapwood.carbon << " " << root.carbon << " " << leaf.carbon << " " << psapwoodlc << " " << cnsapwood[pdcmnt] << endl;
};
/* *************************************************************
************************************************************* */
...
void Tveg45::boundTOPT(const int &pcmnt)
{
  ...
  */
  ...
  if (topt > toptmax[pcmnt])
  {
    topt = toptmax[pcmnt];
  }
  if (toptmic > toptmax[pcmnt])
  {
    toptmic = toptmax[pcmnt];
  }

  if (topt < toptmin[pcmnt])
  {
    topt = toptmin[pcmnt];
  }
  if (toptmic < toptmin[pcmnt])
  {
    toptmic = toptmin[pcmnt];
  }
};
...

{
  ...
  */
  ...
  {
    recovery = (1.0 - fozone) * (1.0 / (12.0 * tauleaf[pdcmnt]) + deltaleafc / leaf.carbon);
  }
  else
  {
    recovery = (1.0 - fozone) * (1.0 / (12.0 * tauleaf[pdcmnt]));
  }
                     ...
                     const double &vegc)
{
  ...
  */
  ...
  // cout << "frdl = " << frdl << " " << kext << " " << kiw << " " << parw << " " << lai << endl;
  ...
  {
    ci = co2;
  } // and ci is basically irrelevant
  ...
  fcanopy = vegc / (vegc + 500.0);
  //  fcanopy =  vegc/(1000.0);
  if (fcanopy > 1.0)
  {
    fcanopy = 1.0;
  }
  //  cout << "fcanopy = " << fcanopy << " " << vegc << endl;

  //  veggpp = cmaxgcmo * frdl * fco2 * temp * fozone * fh2o* fcanopy;
  veggpp = cmaxgcmo * frdl * fco2 * temp * fozone * fh2o;
  //  cout << "veggpp " << veggpp << " " << cmax << " " << cmaxgcmo << " " <<  frdl << " " << fco2 << " " << temp << " " << fozone << " " << fh2o << " " << lai << endl;
  //  cout << "frdl " << frdl << " " << kext << " " << kiw << " " << parw << " " <<  lai << endl;
                        ...
                        const int &pndays)
{
  ...
  */
  ...
  //  if(pdcmnt == 15)
  //   {
  //  seed.rm = krgcmo*rmt*seed.carbon/cnseed[pdcmnt];
  //   }
  //   else
  //   {
  seed.rm = ZERO;
  ...
  if (phi == ZERO)
  {
    ...
    case I_COLD_DEC:
      ...
      break;
    case I_STRESS_DEC:
      leaf.ltrc = 3.0 * leaf.carbon * pndays / (365.0 * tauleaf[pdcmnt]);
                     ...
                     const double &rootmass)
{
  ...
  {
    dvnupdrootc = vegnup * krnup[pdcmnt] / (0.01 * (0.01 + krnup[pdcmnt]));
  }
                          ...
                          const double &prphi)
{
  ...
  */
  ...
  if (phen[pcmnt] == I_STRESS_DEC)
  {
    newtopt = prtairphi / prphi;
  }
  else
  {
    if (tair > newtopt)
  ...
  if (newtopt > toptmax[pcmnt])
  {
    newtopt = toptmax[pcmnt];
  }

  if (newtopt < toptmin[pcmnt])
  {
    newtopt = toptmin[pcmnt];
  }
};
                     ...
                     const double &ptref)
{
  ...
  */
  ...
  // cout << "rmt = " << rmt << " " << ptair << " " << ptref << " " << topt << " " << " " << raqref[pdcmnt] << " " << " " << raalpha[pdcmnt] << " " << rabeta[pdcmnt] << " " << ragamma[pdcmnt] << endl;
};
  ...
  */
  ...
  if (1 == nfeed)
  {
    ...
    if (rdemandn > ZERO && rdemandc > ZERO &&
        ...
        cnprod > ZERO)
    {
      if ((rlabilec / rlabilen > rdemandc / rdemandn) && (labile.carbon / labile.nitrogen > cnprod))
      {
        //    NITROGEN LIMITED
      ...
      else if ((rlabilen / rlabilec > rdemandn / rdemandc) && (labile.nitrogen / labile.carbon > 1.0 / cnprod))
      {
        //    NOT NITROGEN LIMITED = NITROGEN SATURATED
    ...
    // cout << "updateDynamics = " << fgppdownreg << " " << fnupdownreg << " " << ingpp << " " << inuptake << endl;
    //     cout << "gpp = " << gpp << " " << ingpp << " " << fgppdownreg << endl;
    if (gpp < ZERO)
    {
      gpp = ZERO;
    }
