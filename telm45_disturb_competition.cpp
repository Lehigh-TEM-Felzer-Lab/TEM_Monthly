...
20060127 - DWK deleted tem.soil.stm.updateyrCDM(),s
...
#ifdef DEBUGE
  cout << endl
       << " year, vegc, soilc, december prec at equilibrium "
...
void Telm45::outputTEMmonth(const int &pdm, const int &ichrt)
{
  ...
  if (0 == tem.ag.state)
  {
    ...
    output[ichrt][tem.I_NATVEGC][pdm] = tem.getY(tem.I_LEAFC) + tem.getY(tem.I_SAPWOODC) + tem.getY(tem.I_HEARTWOODC) + tem.getY(tem.I_ROOTC) + tem.getY(tem.I_LABILEC);

    output[ichrt][tem.I_CROPN][pdm] = ZERO;
    output[ichrt][tem.I_NATVEGN][pdm] = tem.getY(tem.I_LEAFN) + tem.getY(tem.I_SAPWOODN) + tem.getY(tem.I_HEARTWOODN) + tem.getY(tem.I_ROOTN) + tem.getY(tem.I_LABILEN);
    ...
    output[ichrt][tem.I_NATLTRN][pdm] = tem.getY(tem.I_LTRLN) + tem.getY(tem.I_LTRSN) + tem.getY(tem.I_LTRHN) + tem.getY(tem.I_LTRRN);
  }
  else
  {
    output[ichrt][tem.I_CROPC][pdm] = tem.getY(tem.I_LEAFC) + tem.getY(tem.I_SAPWOODC) + tem.getY(tem.I_HEARTWOODC) + tem.getY(tem.I_ROOTC) + tem.getY(tem.I_LABILEC);
    output[ichrt][tem.I_NATVEGC][pdm] = ZERO;

    output[ichrt][tem.I_CROPN][pdm] = tem.getY(tem.I_LEAFN) + tem.getY(tem.I_SAPWOODN) + tem.getY(tem.I_HEARTWOODN) + tem.getY(tem.I_ROOTN) + tem.getY(tem.I_LABILEN);
    output[ichrt][tem.I_NATVEGN][pdm] = ZERO;

    output[ichrt][tem.I_CSTRN][pdm] = tem.getY(tem.I_LEAFN) + tem.getY(tem.I_SAPWOODN) + tem.getY(tem.I_HEARTWOODN) + tem.getY(tem.I_ROOTN);
    output[ichrt][tem.I_NATSTRN][pdm] = ZERO;
    ...
    output[ichrt][tem.I_AGLTRC][pdm] = tem.getY(tem.I_LTRLC) + tem.getY(tem.I_LTRSC) + tem.getY(tem.I_LTRHC) + tem.getY(tem.I_LTRRC);
    ...
    output[ichrt][tem.I_AGLTRN][pdm] = tem.getY(tem.I_LTRLN) + tem.getY(tem.I_LTRSN) + tem.getY(tem.I_LTRHN) + tem.getY(tem.I_LTRRN);
...

{
  ...
  {
    dyr = de_startyear + 1 - clm.startyr;
  }
  ...
  if (cohort[pichrt].qc != ACCEPT)
  {
    ...
    if (cohort[pichrt].qc < 100 && tem.veg.cmnt > 1 && (mxtair[dyr] < -1.0 || yrprec[dyr] <= ZERO))
    {
  ...
  else // "cohort[pichrt].qc == ACCEPT"
  {
    ...
    ************************************************************* */
    ...
    while (tem.nattempt < tem.maxnrun && 0 == tem.initFlag)
    {
      tem.initializeState();

      tem.nattempt = equilibrateTEM(tem.tol, pichrt, fout, predname, spinoutfg, year, rflog1);

      if (tem.nattempt < tem.maxnrun && 0 == tem.initFlag)
      {
        tem.tol /= 10.0;
      }
    }
                        ...
                        const int &pichrt)
{
  ...
  if (0 == equil)
  {
    ttotyr[pdyr] = tem.startyr - totsptime - 1 - totsptime + (pdyr * tem.diffyr);
  }
  else
  {
    ttotyr[pdyr] = -999;
                          ...
                          const int &equilfg)
{
  ...
  for (i = 0; i < ntempred; ++i)
  {
    ...
    if (equilfg == 1)
    {
      if (predname.at(i) == tem.predstr.at(tem.I_VSM) || predname.at(i) == tem.predstr.at(tem.I_PCTP))
      {
                            ...
                            ofstream &rflog1)
{
  ...
  if (ACCEPT == cohort[pichrt].qc && ACCEPT == cohort[pichrt].tqc)
  {
    ...
    tem.totyr = clm.startyr - totsptime - 1 - totsptime + (pdyr * tem.diffyr);
