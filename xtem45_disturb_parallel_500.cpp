...
#include "telm45_disturb_500.h"          // Telm45 Class
...
//  flog1 << endl << endl;
  ...
  elmnt.show( flog1,
        ...
        {
        for( xdyr = 0; xdyr < (transtime+1); ++xdyr )
        {
       ...
       if( 1 == telmnt[0].lcluc.tlulcflag )
        ...
        for( i = 0; i < (startval); ++i )
        {
        for( xdyr = 0; xdyr < (telmnt[0].lcluc.lastyr - telmnt[0].clm.startyr + 2); ++xdyr )
        {
...
/*  if(lulcdat[tstyr][ichrt].agstate == 0) {
  ...
  else if (lulcdat[tstyr][ichrt].agstate == 1 || (lulcdat[tstyr][ichrt].agstate == 2 )) {
